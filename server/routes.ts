...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/admin/permissions', isAuthenticated, async (req: any, res) => {
    try {
      const { documentId, userId, departmentId, permission = 'read' } = req.body;
      ...
      if (!userId && !departmentId) {
        return res.status(400).json({ message: "Either user ID or department ID is required" });
      }
  ...
  app.delete('/api/admin/permissions/:permissionId', isAuthenticated, async (req: any, res) => {
    try {
...
`;
  ...
  app.get("/api/ai-response-analysis", isAuthenticated, async (req: any, res) => {
    try {
      ...
      const analysis = await storage.getAiResponseAnalysis(userId, { limit, offset, analysisResult });
      res.json(analysis);
    } catch (error) {
  ...
  app.post("/api/ai-response-analysis/analyze", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user.claims.sub;
      const { userQuery, assistantResponse, documentContext, responseTime, chatMessageId } = req.body;
  ...
  app.put("/api/agent-chatbots/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (documentIds !== undefined) {
        console.log("Updating documents for agent:", agentId, "Documents:", documentIds);
        
        // Get current document associations
        const currentDocs = await storage.getAgentChatbotDocuments(agentId, userId);
        const currentDocIds = currentDocs.map(doc => doc.documentId);
        
        // Remove documents that are no longer selected
        for (const currentDocId of currentDocIds) {
          if (!documentIds.includes(currentDocId)) {
            await storage.removeDocumentFromAgent(agentId, currentDocId, userId);
          }
        }
